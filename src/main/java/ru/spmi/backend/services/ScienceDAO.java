...
public class ScienceDAO {
    ...
    public ScienceTableDTO getScienceAllJsonFromFilters(String filters, int page_rows, int page_num) {
        ArrayList<ScienceDTO> scienceList = new ArrayList<>();
        var bdFuncResponse = scienceRepository.scienceFunc( filters, page_rows, page_num);
        bdFuncResponse.forEach(x -> scienceList.add(new ScienceDTO(   x.getCount_Rows(),
                x.getFio_Full(),
        ...
        var scienceTable= new ScienceTableDTO(
                ...
                fieldsList,
                headerName,
                scienceList.stream().findFirst().get().getCount_rows(),
                page_rows,
                page_num
        );
//
