...
import com.usareboot.back.persistence.usareboot.entities.auth.Tab;
...
public class TabAccessService {
     ...
     */
    public List<String> getAllowedTabsForUser(UsersEntity user) {
        ...
        return fullUser.getRoles().stream()
                .flatMap(role -> role.getTabs().stream()) // Получаем Stream<Tab> из всех ролей
                .map(Tab::getPath) // Предполагаем, что у Tab есть поле path (например, "/admin")
